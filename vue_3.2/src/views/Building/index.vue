<template>
  <el-card style="height: 570px; position: relative;">
    <el-row :gutter="20">
      <el-col :span="7">
        <el-input placeholder="请输入楼宇地址或编号" clearable v-model="queryForm.query">
        </el-input>
      </el-col>
...
import { getBuilding, delBuilding, editBuilding } from '@/api/Building'
...
const del = (row) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const res = await delBuilding(row.id)
      ElMessage({
        type: 'success',
        message: '删除成功',
      })
...
const initGetMist = async () => {
  const res = await getBuilding(queryForm.value)
