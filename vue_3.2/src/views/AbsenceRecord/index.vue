...
import { getAbsenceRecord, delAbsenceRecord, editAbsenceRecord } from '@/api/AbsenceRecord'
...
const del = (row) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const res = await delAbsenceRecord(row.id)
      ElMessage({
        type: 'success',
        message: '删除成功',
      })
...
const initGetMist = async () => {
  const res = await getAbsenceRecord(queryForm.value)
