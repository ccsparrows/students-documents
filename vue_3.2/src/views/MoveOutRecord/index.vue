...
import { getMoveoutstudents, delMoveoutstudents, editMoveoutstudents } from '@/api/move_out_records'
...
const del = (row) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const res = await delMoveoutstudents(row.id)
      ElMessage({
        type: 'success',
        message: '删除成功',
      })
...
const initGetMist = async () => {
  const res = await getMoveoutstudents(queryForm.value)
